...
BrowserApplication::BrowserApplication(int &argc, char **argv)
    ...
    , quitting(false)
{
    QCoreApplication::setOrganizationDomain(QLatin1String("arova-browser.org"));
    QCoreApplication::setOrganizationName(QLatin1String("GBonachea"));
    QCoreApplication::setApplicationName(QLatin1String("Arova"));
    ...
    // If we could connect to another Arova then exit
    QString message = QString(QLatin1String("arovamessage://getwinid"));
    if (sendMessage(message.toUtf8(), 500))
        return;

#ifdef BROWSERAPPLICATION_DEBUG
    qDebug() << "BrowserApplication::" << __FUNCTION__ << "I am the only arova";
#endif
    ...
    setWindowIcon(QIcon(QLatin1String(":128x128/arova.png")));
...
void BrowserApplication::messageReceived(QLocalSocket *socket)
{
    ...
    // Got a normal url
    if (!message.startsWith(QLatin1String("arovamessage://"))) {
        QSettings settings;
    ...
    if (message.startsWith(QLatin1String("arovamessage://getwinid"))) {
#ifdef Q_OS_WIN
...
#endif
        QString message = QLatin1String("arovamessage://winid/") + winid;
        socket->write(message.toUtf8());
        socket->waitForBytesWritten();
        return;
    }

    if (message.startsWith(QLatin1String("arovamessage://winid"))) {
...
{
    {
        ...
        if (settings.value(QLatin1String("restoring"), false).toBool()) {
            QMessageBox::StandardButton result = QMessageBox::question(0, tr("Restore failed"),
                tr("Arova crashed while trying to restore this session.  Should I try again?"), QMessageBox::Yes | QMessageBox::No);
...
LanguageManager *BrowserApplication::languageManager()
{
    if (!s_languageManager) {
        ...
        s_languageManager->addLocaleDirectory(qApp->applicationDirPath() + QLatin1String("/locale"));
...
{
    Q_UNUSED(url)
    return QIcon(QLatin1String(":graphics/defaulticon.png"));
}
...
{
    return QWebEngineProfile::defaultProfile()->isOffTheRecord();
}
