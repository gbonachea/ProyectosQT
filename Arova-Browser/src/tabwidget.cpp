...
WebView *TabWidget::makeNewTab(bool makeCurrent)
{
    ...
    if (home == QLatin1String("about:home") || home == QLatin1String("about:blank")) {
        ...
        settings.setValue(QLatin1String("home"), home);
    }
    webView->load(guessUrlFromString(home));
...
void TabWidget::lineEditReturnPressed()
{
    if (QLineEdit *lineEdit = qobject_cast<QLineEdit*>(sender())) {
        ...
        const QString query = lineEdit->text().trimmed();
        if (!query.isEmpty())
            loadString(query, tab);
...
void TabWidget::windowCloseRequested()
{
    ...
    if (!webPage)
        return;
    WebView *webView = qobject_cast<WebView*>(webPage->parent());
...
QUrl TabWidget::guessUrlFromString(const QString &string)
{
    ...
    if (trimmed == QLatin1String("about:home") || trimmed == QLatin1String("about:blank"))
        return QUrl(QLatin1String("qrc:/page.html"));
    ...
    if (url.scheme() == QLatin1String("about")
        ...
        url = QUrl(QLatin1String("qrc:/page.html"));
    ...
    QString urlString = QLatin1String("http://") + trimmed;
