    ...
    , m_page(new WebPage(this))
{
    ...
    connect(page()->profile(), &QWebEngineProfile::downloadRequested,
            this, &WebView::downloadRequested);
...
void WebView::loadUrl(const QUrl &url, const QString &title)
{
    if (url.scheme() == QLatin1String("javascript")) {
        QString scriptSource = QUrl::fromPercentEncoding(url.toString(QUrl::RemoveScheme).toLatin1());
...
void WebView::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    m_page->event(event);
    if (!event->isAccepted()
        && (BrowserApplication::instance()->eventMouseButtons() & Qt::MiddleButton)) {
...
{
    BrowserApplication::downloadManager()->download(download->url());
}
