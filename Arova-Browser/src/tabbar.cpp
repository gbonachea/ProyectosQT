...
TabBar::TabBar(QWidget *parent)
    ...
    , m_addTabIcon(QLatin1String(":graphics/addtab.png"))
{
...
QRect TabBar::addTabButtonRect() const
{
    int buttonSize = 24;
    int spacing = 4;
    int x;
    if (count() > 0) {
        ...
        x = lastTab.right() + spacing;
        // If last tab is scrolled/offscreen, place at right edge
        if (x + buttonSize > width())
            x = width() - buttonSize - spacing;
    } else {
        x = spacing;
    }
    return QRect(x, (height() - buttonSize) / 2, buttonSize, buttonSize);
}
...
void TabBar::paintEvent(QPaintEvent *event)
{
    ...
    QPainter p(this);
    m_addTabRect = addTabButtonRect();

    QStyleOptionToolButton opt;
    opt.initFrom(this);
    opt.rect = m_addTabRect;
    opt.icon = m_addTabIcon;
...
void TabBar::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        ...
        if (m_addTabRect.isValid() && m_addTabRect.contains(event->pos())) {
            emit newTab();
            return;
        }
    }
