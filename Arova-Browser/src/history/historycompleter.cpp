...
HistoryCompletionModel::HistoryCompletionModel(QObject *parent)
    ...
    , m_isValid(false)
{
    m_searchMatcher.setPatternOptions(QRegularExpression::CaseInsensitiveOption);
...
void HistoryCompletionModel::setSearchString(const QString &str)
{
    ...
    m_searchString = str;
    m_searchMatcher.setPattern(QRegularExpression::escape(str));
...
bool HistoryCompletionModel::filterAcceptsRow(int source_row, const QModelIndex &source_parent) const
{
    ...
    if (m_searchMatcher.match(url).hasMatch())
        return true;

    QString title = sourceModel()->data(idx, HistoryModel::TitleRole).toString();

    if (m_searchMatcher.match(title).hasMatch())
        return true;
...
{
    QString url = model()->data(index, HistoryModel::UrlStringRole).toString();
    if (!url.isEmpty())
        return url;
