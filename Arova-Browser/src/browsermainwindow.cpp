...
void BrowserMainWindow::loadDefaultState()
{
    ...
    restoreState(data);
    if (settings.contains(QLatin1String("size"))) {
        resize(settings.value(QLatin1String("size")).toSize());
        move(settings.value(QLatin1String("pos")).toPoint());
    }
    if (settings.value(QLatin1String("maximized"), false).toBool())
        setWindowState(windowState() | Qt::WindowMaximized);
    if (settings.value(QLatin1String("fullScreen"), false).toBool())
...
void BrowserMainWindow::aboutToShowTextEncodingMenu()
{
    ...
    QStringList codecs = QStringList()
        ...
        << QStringLiteral("ISO-8859-15") << QStringLiteral("ISO-8859-16")
        << QStringLiteral("KOI8-R") << QStringLiteral("KOI8-U")
        << QStringLiteral("GBK") << QStringLiteral("GB2312")
        << QStringLiteral("Big5") << QStringLiteral("Big5-HKSCS")
        << QStringLiteral("Shift_JIS") << QStringLiteral("EUC-JP")
        << QStringLiteral("EUC-KR") << QStringLiteral("UTF-8")
        << QStringLiteral("UTF-16") << QStringLiteral("UTF-32")
    ...
    QString defaultTextEncoding = QWebEngineProfile::defaultProfile()->settings()->defaultTextEncoding();
...
void BrowserMainWindow::viewTextEncoding(QAction *action)
{
    ...
    if (codec.isEmpty())
        QWebEngineProfile::defaultProfile()->settings()->setDefaultTextEncoding(QString());
    else
        QWebEngineProfile::defaultProfile()->settings()->setDefaultTextEncoding(codec);
}
...
#else
        setWindowTitle(tr("%1 - Arova", "Page title and Browser name").arg(title));
#endif
    }
}
...
void BrowserMainWindow::printRequested()
{
    if (!currentTab())
        return;
    currentTab()->page()->printToPdf([this](const QByteArray &result) {
        if (result.isEmpty())
            return;
        QString fileName = QFileDialog::getSaveFileName(this, tr("Save PDF"), QString(), QLatin1String("*.pdf"));
        if (fileName.isEmpty())
            return;
        QFile file(fileName);
...
void BrowserMainWindow::goHome()
{
    QSettings settings;
    settings.beginGroup(QLatin1String("MainWindow"));
    QString home = settings.value(QLatin1String("home"), QLatin1String("qrc:/page.html")).toString();
    if (home == QLatin1String("about:home") || home == QLatin1String("about:blank"))
        home = QLatin1String("qrc:/page.html");
...
void BrowserMainWindow::aboutToShowBackMenu()
{
    ...
    if (!currentTab())
        return;
    auto history = currentTab()->history();
    int historyCount = history->count();
    for (int i = history->backItems(historyCount).count() - 1; i >= 0; --i) {
        auto item = history->backItems(history->count()).at(i);
...
void BrowserMainWindow::aboutToShowForwardMenu()
{
    ...
    for (int i = 0; i < history->forwardItems(history->count()).count(); ++i) {
        auto item = history->forwardItems(historyCount).at(i);
        QAction *action = new QAction(this);
...
void BrowserMainWindow::aboutToShowWindowMenu()
{
    ...
    QAction *downloadManagerAction = m_windowMenu->addAction(tr("Downloads"), QKeySequence(tr("Ctrl+Y", "Download Manager")), this, SLOT(downloadManager()));
