...
void SourceHighlighter::highlightBlock(const QString &text)
{
    ...
    while (pos >= 0 && pos < len && len > 0) {
        ...
        default:
        case Normal: {
            QRegularExpressionMatch match = QRegularExpression(QLatin1String("[<&]")).match(text, pos);
            ...
            if (pos >= 0) {
                if (text.at(pos) == QLatin1Char('<')) {
                    ...
                    QRegularExpressionMatch entityMatch = QRegularExpression(QLatin1String("&[a-zA-Z0-9]+;")).match(text, pos);
                    if (entityMatch.hasMatch() && entityMatch.capturedStart() == pos)
                        setFormat(pos, entityMatch.capturedLength(), formats[Entity]);
                    ++pos;
                }
            }
            break;
