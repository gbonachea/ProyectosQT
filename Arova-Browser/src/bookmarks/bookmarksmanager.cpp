...
void BookmarksManager::importBookmarks()
{
        ...
        if (process.error() != QProcess::UnknownError) {
            if (process.error() == QProcess::FailedToStart) {
                QMessageBox::warning(0, tr("htmlToXBel tool required"),
                    tr("htmlToXBel tool, which is shipped with Arova and is needed to import HTML bookmarks, "
    ...
    importRootNode->title = (tr("Imported %1").arg(QDate::currentDate().toString(QLocale().dateFormat(QLocale::ShortFormat))));
